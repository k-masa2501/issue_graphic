...
class StackedBarChartsController < ApplicationController
  ...
  def daily_data(params)
    ...
    case params[:f_method]
      when 'sum_act_value' then # 消化工数の推移
        result = data_formalize(trend_of_total_progress(days, get_map(params), '%Y/%m/%d'))
      when 'count_act_value' then # 消化件数の推移
        result = data_formalize(trend_of_total_count(days, get_map(params), '%Y/%m/%d'))
      when 'ticket_amount' then  # チケット量の推移
        result = data_formalize(ticket_amount(days, get_map(params), '%Y/%m/%d'))
      when 'workload' then # 作業量の推移
        result = data_formalize(workload(days, get_map(params), '%Y/%m/%d'))
      when 'per_period_work' then # 期間ごとの作業量
        result = data_formalize(per_period_work_day(days, get_map(params), '%Y/%m/%d'))
      when 'per_period_oc' then # 期間ごとの発生/クローズ件数
        result = data_formalize_period(per_period_oc_day(days, get_map(params), '%Y/%m/%d'))
      else
    end
  ...
  def weekly_data(params)
    ...
    case params[:f_method]
      ...
      when 'per_period_work' then # 単位ごとの作業量
        result = data_formalize(per_period_work_week(days, get_map(params), '%Y/%m/%d'))
      when 'per_period_oc' then # 期間ごとの発生/クローズ件数
        result = data_formalize_period(per_period_oc_week(days, get_map(params), '%Y/%m/%d'))
      else
    end
  ...
  def monthly_data(params)
    ...
    case params[:f_method]
      when 'sum_act_value' then # 消化工数の推移
        result = data_formalize(trend_of_total_progress(days, get_map(params), '%Y/%m'))
      when 'count_act_value' then # 消化件数の推移
        result = data_formalize(trend_of_total_count(days, get_map(params), '%Y/%m'))
      when 'ticket_amount' then  # チケット量の推移
        result = data_formalize(ticket_amount(days, get_map(params), '%Y/%m'))
      when 'workload' then # 作業量の推移 workload
        result = data_formalize(workload(days, get_map(params), '%Y/%m'))
      when 'per_period_work' then # 単位ごとの作業量
        result = data_formalize(per_period_work_month(days, get_map(params), '%Y/%m'))
      when 'per_period_oc' then # 期間ごとの発生/クローズ件数
        result = data_formalize_period(per_period_oc_month(days, get_map(params), '%Y/%m'))
      else
    end
  ...
  def trend_of_total_progress(days, map, format)
    ...
    (0 < data.length) and days.each do |v1|
      ...
      if 0 < (v2 = data.select {|v2| Date.parse(v1).strftime(format) == Date.parse(v2[0]).strftime(format) }).length
        v2.each do |v3|
          result.push([v3[0], v3[1], v3[2]])
        end
      else
        result.push([Date.parse(v1).strftime(format), keys[0], 0])
      end
  ...
  def per_period_work(days, map, format, condition)
    ...
    days.each do |v|
      sql.push("("+record.select("DATE_FORMAT(#{v},'#{format}')",query_var1,"ifnull(sum(aggregations.progress), 0)")
  ...
  def per_period_work_week(days, map, format)
    condition = ->(v){ return "aggregations.today BETWEEN #{v} AND '#{(Date.parse(v)+6)}'"}
    per_period_work(days, map, format, condition)
  end

  def per_period_work_month(days, map, format)
    condition = ->(v){ return "aggregations.today BETWEEN '#{Date.parse(v).beginning_of_month}' AND '#{(Date.parse(v))}'"}
    per_period_work(days, map, format, condition)
  end
  ...
  def per_period_oc(days, map, format, condition)
    ...
    keys1 = {keyword[0] => "#{keyword[0]}・・・発生件数", keyword[1] => "#{keyword[1]}・・・クローズ件数"}
    query_var1 = "ifnull(#{map[:view][:pluck]}, 'null')"
    ...
    days.each do |v|
      ...
      sql.push("("+record.select("DATE_FORMAT(#{v},'#{format}')",
                                 ...
                                 query_var1)
                       .where(open_between+" or "+close_between)
                       .group(query_var1).to_sql+")")
    end
    ...
    return {:data => result, :keys1 => keys1, :keys2 => keys2}
  ...
  def per_period_oc_day(days, map, format)
    ...
    condition = ->(v){
      fmt = '%Y/%m/%d %H:%M'
      return "BETWEEN '#{DateTime.parse(v+' 00:00').strftime(fmt)}' and '#{DateTime.parse(v+' 23:59').strftime(fmt)}'"
    }
  ...
  def per_period_oc_week(days, map, format)
    ...
    condition = ->(v){
      fmt = '%Y/%m/%d %H:%M'
      return "BETWEEN '#{DateTime.parse(v+' 00:00').strftime(fmt)}' and '#{(DateTime.parse(v+' 23:59')+6).strftime(fmt)}'"
    }
    per_period_oc(days, map, format, condition)
  ...
  def per_period_oc_month(days, map, format)
    ...
    condition = ->(v){
      fmt = '%Y/%m/%d %H:%M'
      return "BETWEEN '#{DateTime.parse(v+' 00:00').beginning_of_month.strftime(fmt)}' and '#{DateTime.parse(v+' 23:59').strftime(fmt)}'"
    }
    per_period_oc(days, map, format, condition)
